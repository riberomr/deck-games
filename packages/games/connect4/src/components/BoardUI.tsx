...
import { CellChecker, Player, GameStatus } from '../hooks/useGameEngine';
...
interface BoardUIProps {
    ...
    playColumn: (colIndex: number) => void;
    resetGame?: () => void;
    showResetButton?: boolean;
    playerColor?: Player; // For online: "You are Red"
}
    ...
    return (
        <div className="flex flex-col items-center gap-6 p-4 w-full max-w-lg mx-auto">
            ...
            <div className="flex justify-between items-center w-full px-4 py-3 bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-800">
                ...
                {playerColor && (
                    <div className="text-sm font-medium text-zinc-600 dark:text-zinc-400">
                        You: <span className={`font-bold uppercase ${playerColor === 'red' ? 'text-red-500' : 'text-yellow-500'}`}>{playerColor}</span>
                    </div>
                )}
                        ...
                        row.map((cell: CellChecker, colIndex: number) => {
                            ...
                            return (
                                <button
                                    ...
                                    disabled={status !== 'playing' || !!board[0][colIndex] || (status === 'playing' && playerColor && currentPlayer !== playerColor)}
                                    className={`group relative w-10 h-10 md:w-14 md:h-14 flex items-center justify-center focus:outline-none 
                                        ${(status === 'playing' && playerColor && currentPlayer !== playerColor) ? 'cursor-not-allowed opacity-80' : ''}
                                    `}
